            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  <span className={`text-sm ${scrolled ? "text-muted-foreground" : "text-white/80"}`}>
                    {user.email?.split("@")[0]}
                  </span>
                  <button
                    onClick={() => signOut()}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all ${scrolled ? "text-muted-foreground hover:bg-muted" : "text-white/80 hover:bg-white/10"}`}
                  >
                    <LogOut size={14} />
                  </button>
                </div>
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${scrolled ? "text-muted-foreground hover:bg-muted" : "text-white/80 hover:bg-white/10"}`}
                >
                  <User size={14} />
                  Login
                </button>
              ...
              <div className="container-wide py-6 space-y-2">
                ...
                {user ? (
                  <button
                    ...
                    className="block w-full text-left py-4 text-lg font-medium text-foreground hover:text-primary border-b border-border/50"
                  >
                    Sign Out
                  </button>
                ) : (
